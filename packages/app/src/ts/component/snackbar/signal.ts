...
const logger = createLogger('component/snackbar');
...
export const snackbarActionButtonClickedSignal = new AlwatrTrigger({
  ...
  loggerPrefix: 'snackbar',
});

/**
 * Signal for displaying the snackbar.
 */
export const snackbarSignal = new AlwatrSignal<SnackbarOptions>({name: 'snackbar', loggerPrefix: 'snackbar'});
...
async function showSnackbar(options: SnackbarOptions): Promise<void> {
  ...
  if (options.duration !== -1) {
    delay.by(options.duration).then(closeSnackbar_);
  }
}
